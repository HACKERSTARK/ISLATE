...
def translate_and_speak(text, lang_code):
    try:
        ...
        messagebox.showerror("Translation/Speech Error", str(e))

# === APP CLASS ===
class HandDetectorApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Hand Detection Translator + Voice")
        ...
        lang_frame.pack()
        Label(lang_frame, text="Translate & Speak In: ").pack(side=LEFT)
        self.language_var = StringVar(value="English")
        lang_dropdown = ttk.Combobox(lang_frame, textvariable=self.language_var, values=list(LANGUAGES.keys()), state="readonly")
    ...
    def update_video(self):
        ...
        if len(detections['detection_scores']) > 0 and detections['detection_scores'][0] > 0.8:
            ...
            if label == 'enter':
                if self.enter_detected_time and current_time - self.enter_detected_time >= 1.0:
                    self.speak_string()
                    self.enter_detected_time = None
                else:
                    self.enter_detected_time = current_time
    ...
    def speak_string(self):
        ...
        lang_code = LANGUAGES.get(lang_name, "hi")
        if self.final_string.strip():
            threading.Thread(target=translate_and_speak, args=(self.final_string, lang_code)).start()
